...
function Player:update(dt)
    ...
    if self.x <= -32 and self.y >= 204 then
        self.x = map.tileHeight * ((map.mapHeight - 2) / 2) - self.height
        self.y = map.tileWidth * 10
    elseif self.x >= (map.mapWidth - 1) * map.tileWidth then
...
function Player:render()
    ...
    love.graphics.printf('x: ' ..tostring(self.x), 0, 42, VIRTUAL_WIDTH, 'center')
    love.graphics.printf('y: '..tostring(self.y), 0, 50, VIRTUAL_WIDTH, 'center')
