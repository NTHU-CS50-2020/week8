...
function Map:init()
    ...
    while x < self.mapWidth do
        ...
        if x >= self.mapWidth - 5 then
            ...
            x = x + 1
        elseif x >= self.mapWidth - 10 then
            -- creates column of tiles going to bottom of map
            for y = self.mapHeight / 2 - pyramidOffset, self.mapHeight do
                self:setTile(x, y, TILE_BRICK)
            end

            -- next vertical scan line
            x = x + 1
            pyramidOffset = math.min(pyramidOffset + 1, 4)
    ...
    self:setTile(x, 4, FLAG_TOP)
    for y = 5, self.mapHeight / 2 - 1 do
